...
def computer_shoots(set_to_shoot_from):
    ...
    compueter_available_to_fire_set.discard(computer_fired_block) #записываем эту клетку
...
#обнавление списка со всеми точками и обновление множества, в котором находятся клетки с точками у человека (для понимания комп. куда не нужно стрелять)
